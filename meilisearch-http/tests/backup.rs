...
async fn trigger_backup_twice_should_return_conflict() {
    ...
    let expected = json!({
        "message": "Another backup is already in progress, please try again later",
...
async fn trigger_backup_concurently_should_return_conflict() {
    ...
    let ((value_1, status_code_1), (value_2, status_code_2)) = futures::join!(server.trigger_backup(), server.trigger_backup());
...
async fn backup_metadata_should_be_valid() {
    ...
    let backup_path = Path::new(&server.data().backup_path);
    let tmp_dir = TempDir::new().unwrap();
    let tmp_dir_path = tmp_dir.path();

    compression::from_tar_gz(&backup_path.join(&format!("{}.tar.gz", uid)), tmp_dir_path).unwrap();
...
async fn backup_gzip_should_have_been_created() {
    ...
    let compressed_path = backup_path.join(format!("{}.tar.gz", backup_uid));
