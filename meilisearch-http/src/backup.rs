...
use heed::RwTxn;
use log::error;
use meilisearch_core::{MainT, UpdateT};
...
// Mutex to share backup advancement.
...
fn import_index_v1(
    ...
    let index_path = &backup_path.join(index_uid);
    ...
    for document in documents {
        // convert document in `IndexMap`
        let vec = serde_json::to_vec(&document?)?;
        let document = serde_json::from_slice(&vec)?;
...
/// Infer backup_path from backup_uid
pub fn compressed_backup_path(backup_path: &Path, backup_uid: &str) -> PathBuf {
    backup_path.join(format!("{}.tar.gz", backup_uid))
}
...
fn backup_index_settings(data: &web::Data<Data>, reader: &RoTxn<MainT>, folder_path: &Path, index_uid: &str) -> Result<(), Error> {
...
fn backup_index_updates(data: &web::Data<Data>, reader: &RoTxn<UpdateT>, folder_path: &Path, index_uid: &str) -> Result<(), Error> {
...
fn backup_index_documents(data: &web::Data<Data>, reader: &RoTxn<MainT>, folder_path: &Path, index_uid: &str) -> Result<(), Error> {
    ...
    let mut offset = 0;
    loop {
        let documents = crate::routes::document::get_all_documents_sync(data, reader, index_uid, offset, limit, None)?;
        if documents.len() == 0 { break; } else { offset += limit; }
...
fn backup_process(data: web::Data<Data>, backup_path: PathBuf, backup_info: BackupInfo) {
    ...
    if let Err(e) = crate::helpers::compression::to_tar_gz(&tmp_dir_path, &compressed_backup_path(&backup_path, &backup_info.uid)) {
...
pub fn init_backup_process(data: &web::Data<Data>, backup_path: &Path) -> Result<BackupInfo, Error> {
    create_dir_all(backup_path).or(Err(Error::backup_failed()))?;
...

    {
        ...
        thread::spawn(move || 
            backup_process(data, backup_path, resume)
        );
    }
